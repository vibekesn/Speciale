...
fileName <- paste(path_out, "AAV_17.10.csv", sep = "")
Date <- "17.10.2022"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/DATALOGaalborgV17.10-09.11.xlsx", sheet = "17.10")
```
...
ggplot(blanketheight, aes(Timestamp, Height)) + 
  ...
  scale_y_continuous(breaks = round(seq(min(0), max(310), by = 20)))

to2000s <- blanketheight[blanketheight$Timestamp <2000, ]
ggplot(to2000s, aes(Timestamp, Volume)) + 
  geom_point()+ labs(x="Time [s]", y="Sludge blanket height [mL]" ) +
  scale_y_continuous(breaks = round(seq(min(0), max(1100), by = 50)))
...
SV30_sub <- SV30_sub[SV30_sub$Volume >150, ]
