...
fileName <- paste(path_out, "AAV_09.11.csv", sep = "")
Date <- "09.11.2022"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/DATALOGaalborgV17.10-09.11.xlsx", sheet = "09.11")
```
...
```{r}
#plot 
blanketheight <- blanketheight[blanketheight$Timestamp <2000, ]
blanketheight <- blanketheight[blanketheight$Height <300, ]

ggplot(blanketheight, aes(Timestamp, Height)) + 
  geom_point()+ labs(x="Time [s]", y="Sludge blanket height [mm]", )+scale_x_continuous(breaks = round(seq(min(0), max(2000), by = 100)))


ggplot(blanketheight, aes(Timestamp, Volume)) + 
  geom_point()+ labs(x="Time [s]", y="Sludge blanket height [mL]" ) +
  scale_y_continuous(breaks = round(seq(min(0), max(1100), by = 100)))
```
...
hin_settling <- blanketheight[blanketheight$Height <293, ]
...
SV30_sub <- SV30_sub[SV30_sub$Volume >155, ]
...
parameterName <- paste(path_out, "AAV_parameters.csv", sep = "")
