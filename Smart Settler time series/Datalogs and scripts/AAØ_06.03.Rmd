...
fileName <- paste(path_out, "AAØ_06.03.csv", sep = "")
Date <- "06.03.2023"
```
...
#Defining parameters
```{r}
AAØ_metadata <- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/Aalborg Øst metadata.xlsx", sheet = "Ark1")
SS <- AAØ_metadata[(AAØ_metadata[,"Dato"]) == Date, "SS"]   
Dilution.percent <-AAØ_metadata[(AAØ_metadata[,"Dato"]) == Date, "Fortynding"]
Dilution.percent <- Dilution.percent*0.1
n <- 100/Dilution.percent #Dilution_factor
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/DATALOGaaø28.02-27.03.xlsx", sheet = "06.03")
```
...
hin_settling <- hin_settling[hin_settling$Height >85, ]


#find difference between every 5th row (70 s)
diff_by5 <- diff(as.matrix(hin_settling$Height), lag = 15)
...
max_slope_span <- max_diff_index +15L
...
SV30_sub <- blanketheight[blanketheight$Timestamp >750, ]
SV30_sub <- SV30_sub[SV30_sub$Timestamp <2000, ]
#removing scewing datapoints
SV30_sub <- SV30_sub[SV30_sub$Height >85, ]
...
parameterName <- paste(path_out, "AAØ_parameters.csv", sep = "")
```
