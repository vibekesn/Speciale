...
fileName <- paste(path_out, "DAM_09.01.csv", sep = "")
Date <- "09.01.2023"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/Biofos09.01.xlsx", sheet= "09.01")
...
SS <- as.numeric(SS)
Dilution.percent <-40 #[%]
n <- Biofos_metadata[(Biofos_metadata[,"Date"]) == Date, "Dilutionfactor"] #Dilution_factor
```
...
blanketheight <- blanketheight[blanketheight$Volume >68, ]
...
hin_settling <- hin_settling[hin_settling$Height >219, ]
...
SV30_sub <- SV30_sub[SV30_sub$Height >219, ]
