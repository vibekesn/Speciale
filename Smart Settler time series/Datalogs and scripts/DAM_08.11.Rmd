...
fileName <- paste(path_out, "DAM_08.11.csv", sep = "")
Date <- "08.11.2022"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Time_series/Biofos20.10-15.11.xlsx", sheet = "08.11")
```
...
ggplot(blanketheight, aes(Timestamp, Height)) + 
  geom_point()+ labs(x="Time [s]", y="Sludge blanket height [mm]")+ ylim(0,350)
...
ggplot(blanketheight, aes(Timestamp, Height)) + 
  geom_point()+ 
  labs(x="Time [s]", y="Sludge blanket height [mm]")+ ylim(0,350)+ geom_abline(intercept = Velocity.intercept, slope = velocity.slope,color="blue")
...
SV30_sub <- blanketheight[blanketheight$Timestamp >1700, ]
