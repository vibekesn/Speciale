...
fileName <- paste(path_out, "Randers100.csv", sep = "")
testName <- "Randers"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Broad_inspection/DATALOGranders.xlsx", sheet = "100")
```
...
ggplot(blanketheight, aes(Timestamp, Volume)) + 
  geom_point()+ labs(x="Time [s]", y="Sludge volume [mL]" )+
...
#hin_settling <- hin_settling[hin_settling$Height >150, ]
...
#hin_settling <- hin_settling[hin_settling$Timestamp <400, ]
...
#SV30_sub <- SV30_sub[SV30_sub$Height >215, ]
...
ggplot(blanketheight, aes(Timestamp, Volume)) + 
  geom_point()+ 
  labs(x="Time [s]", y="Sludge mirror height [mL]")+
  geom_abline(intercept = SV_30_intercept, slope = SV_30_slope, color = "blue")+
  scale_y_continuous(breaks = c(100,200,300,400,500,600,700,800,900,1000,1100))+
  scale_x_continuous(breaks = c(200,400,600,800,1000,1200,1400,1600,1800,2000))
```
