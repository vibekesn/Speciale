...
fileName <- paste(path_out, "mariagerfjord75.csv", sep = "")
testName <- "Mariagerfjord"
```
...
```{r}
d<- openxlsx::read.xlsx( "D://Speciale/Data/Broad_inspection/DATALOGmariagerfjord.xlsx", sheet = "75")
```
...
heatmap(d1_matrix, Colv=NA, Rowv=NA,scale = "column", 
        ylab = "Tid [s]", xlab= "Sensor", main = "Rådata")
...
ggplot(blanketheight, aes(Timestamp, Height)) + 
  geom_point()+ 
  labs(x="Tid [s]", y="Slamspejlshøjde [mm]")+ 
  geom_abline(intercept = Velocity.intercept, slope = velocity.slope,color="blue")+
  ggtitle("Bundfældningskurve")+
  scale_y_continuous(breaks = c(0,25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300))+
  scale_x_continuous(breaks = c(0,300,600, 900, 1200, 1500, 1800, 2100))
...
#hin_settling <- hin_settling[hin_settling$Timestamp <250, ]
...
$DSVI=(SV_{30}*n)/SS$ 
```{r}
 blanketheight<- blanketheight[blanketheight$Volume <1000, ]
#visualize volume to time
ggplot(blanketheight, aes(Timestamp, Volume)) + 
  geom_point()+ 
  labs(x="Tid [s]", y="Slamspejlshøjde [mL]", )+
  ggtitle("Bundfældningskurve")+
  scale_y_continuous(breaks = c(0,100,200,300,400,500,600, 700, 800, 900, 1000))+
  scale_x_continuous(breaks = c(0,300,600, 900, 1200, 1500, 1800, 2100))+ geom_abline(intercept = 1122.11054542543, slope = -3.19057938217351
,color="blue")



#Create subset for data at 1800 s
SV30_sub <- blanketheight[blanketheight$Timestamp >1600, ]
SV30_sub <- SV30_sub[SV30_sub$Timestamp <2000, ]
#removing scewing datapoints
#SV30_sub <- SV30_sub[SV30_sub$Height >20, ]
